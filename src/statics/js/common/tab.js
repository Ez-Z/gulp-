layui.define(['element', 'common'], function (exports) {
    ...
    Tab.prototype.tabAdd = function (data) {
        ...
        if (tabIndex === -1) {
            if (_config.openWait) {
                waitLoadIndex = layer.msg('正在加载请稍后...', {icon: 6});;
            }
            //设置只能同时打开多少个tab选项卡
            ...
            if (data.icon !== undefined) {
                if (data.icon.indexOf('qbm-icon-') !== -1) {
                    title += '<i class="qbm-icon ' + data.icon + '" aria-hidden="true"></i>';
                } else {
            ...
            ELEM.contentBox.find('iframe[data-id=' + globalTabIdIndex + ']').each(function () {
                $(this).height(ELEM.contentBox.height()+6);//删除版权信息+33
            });
